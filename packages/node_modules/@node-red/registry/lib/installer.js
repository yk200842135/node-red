...
async function installModule(module,version,url) {
    ...
    activePromise = activePromise.then(async function() {
        ...
        var args = ['install', '--unsafe-perm', '--no-audit', '--no-update-notifier','--no-fund','--save','--save-prefix=~','--production',installName];
...
function uninstallModule(module) {
    activePromise = activePromise.then(() => {
        return new Promise((resolve,reject) => {
            ...
            var args = ['remove', '--unsafe-perm', '--no-audit','--no-update-notifier','--no-fund','--save',module];
            log.trace(npmCommand + JSON.stringify(args));
